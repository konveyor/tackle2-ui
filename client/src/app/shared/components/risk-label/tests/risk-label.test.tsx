...
describe("RiskLabel", () => {
  it("Green", () => {
    ...
    expect(container.firstChild).toHaveClass("pf-v5-c-label pf-v5-m-green");
    expect(screen.getByText("risks.low")).toBeInTheDocument();
  });
  it("Amber", () => {
    const { container } = render(<RiskLabel risk="AMBER" />);
    expect(container.firstChild).toHaveClass("pf-v5-c-label pf-v5-m-orange");
    expect(screen.getByText("risks.medium")).toBeInTheDocument();
  });
  it("Red", () => {
    const { container } = render(<RiskLabel risk="RED" />);
    expect(container.firstChild).toHaveClass("pf-v5-c-label pf-v5-m-red");
