...
export class Assessment {
  ...
  public static selectMigrationAction(risk: string): void {
    ...
    if (risk === "low") {
      ...
      action =
        migrationActions[Math.floor(Math.random() * migrationActions.length)];
    } else if (risk === "medium") {
  ...
  public static selectEffortEstimate(risk: string): void {
    ...
    if (risk === "low") {
      ...
      effort =
        effortEstimates[Math.floor(Math.random() * effortEstimates.length)];
    }
      ...
      .then((count) => {
        ...
        for (let i = 0; i <= lastStep; i++) {
          cy.get(questionBlock).each(($question) => {
            const totalOptions = $question.find(stack).children("div").length;
            ...
            cy.get("span.pf-v5-c-label__text").each((item) => {
              if (Cypress.$(item).text().includes(name)) {
                ...
                if (list[i] == "Effort estimate")
                  expect(Cypress.$(item).text()).to.be.oneOf(
                    effortEstimateList
                  );
                if (
                  list[i] == "Business criticality" ||
                  list[i] == "Work priority"
                )
  ...
  public static validateAssessmentField(
    name: string,
    page: string,
    risk: string
  ): void {
