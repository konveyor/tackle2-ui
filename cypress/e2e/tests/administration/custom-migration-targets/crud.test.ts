    ...
    Object.values(Languages).forEach((language) => {
      describe(`Custom Migration Targets CRUD | ${language} language`, function () {
        ...
        it("Custom Migration Targets CRUD with rules uploaded manually", function () {
          const targetData =
            this.customMigrationTargets[`${languageLower}_manual_rules`];
          ...
          target.create();
          cy.contains(
            CustomMigrationTargetView.takeMeThereNotification
          ).click();
          closeSuccessAlert();
        ...
        it("Create Custom Migration Target with rules from repository with credentials", function () {
          ...
          const targetData =
            this.customMigrationTargets["rules_from_tackle_testApp"];
        ...
        it("Change layout and check in analysis wizard", function () {
          ...
          const dragButton = cy
            .contains(CustomMigrationTargetView.card, target.name, {
              timeout: 12 * SEC,
            })
          ...
          cy.dragAndDrop(
            dragButton,
            cy.get(CustomMigrationTargetView.cardContainer)
          );
          cy.wait("@getTargets");
        ...
        it("Custom rule with source and target technology", function () {
          const targetData =
            this.customMigrationTargets["rules_with_source_target_element"];
          const target = new CustomMigrationTarget(
            data.getRandomWord(8),
            data.getDescription(),
            targetData.image,
