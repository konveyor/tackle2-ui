...
describe(["@tier1"], "Affected files validation", () => {
  ...
  it("Affected files validation with source analysis on bookserver app", function () {
    ...
    const application = new Analysis(
      ...
      getRandomAnalysisData(
        this.analysisData["affected_files_on_bookserverapp"]
      )
    );
    application.create();
    ...
    application.validateIssues(
      this.analysisData["affected_files_on_bookserverapp"]["issues"]
    );
  ...
  it("Affected files validation with source analysis on tackle-testapp app", function () {
    ...
    const application = new Analysis(
      ...
      getRandomAnalysisData(
        this.analysisData["affected_files_on_tackleTestapp"]
      )
    );
    application.create();
    cy.wait("@getApplication");
    ...
    application.verifyAnalysisStatus("Completed");
    application.validateIssues(
      this.analysisData["affected_files_on_tackleTestapp"]["issues"]
    );
  ...
  it("Bug Tackle-1110:Affected files validation with source+deps analysis on tackle-testapp app", function () {
    // https://github.com/konveyor/analyzer-lsp/issues/1110
    ...
    const application = new Analysis(
      ...
      getRandomAnalysisData(
        this.analysisData["affected_files_on_tackleTestapp_deps"]
      )
    );
    application.create();
    cy.wait("@getApplication");
  ...
  it("Affected files validation with source+deps analysis on coolStore app", function () {
    ...
    application.verifyEffort(
      this.analysisData["affected_files_on_coolStore_deps"]["effort"]
    );
    application.validateIssues(
      this.analysisData["affected_files_on_coolStore_deps"]["issues"]
    );
