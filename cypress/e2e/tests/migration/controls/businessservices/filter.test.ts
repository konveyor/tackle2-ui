...
describe(
  ["@tier3", "@tier3_C"],
  "Business services filter validations",
  function () {
    before("Login and Create Test Data", function () {
      login();
      cy.visit("/");
      stakeholdersList = createMultipleStakeholders(3);
      businessServicesList = createMultipleBusinessServices(
        2,
        stakeholdersList
      );
    });
    ...
    it("Description filter validations", function () {
      ...
      const validSearchInput = businessServicesList[0].description.substring(
        0,
        8
      );
