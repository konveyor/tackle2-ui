...
describe(["@tier2"], "Custom Rules RBAC operations", function () {
  ...
  it("Bug Tackle-966: Admin, Rules from public repository", function () {
  ...
  it("Bug Tackle-966: Admin, Rules from private repository with credentials", function () {
  ...
  it("Bug Tackle-966: Architect, Rules from public and private(with and without credentials) repository", function () {
    architect.login();
    analyzeAndVerify(analysisWithPublicRules, AnalysisStatuses.completed);
    analyzeAndVerify(analysisWithPrivateRules, AnalysisStatuses.completed);
    analyzeAndVerify(analysisWithPrivateRulesNoCred, AnalysisStatuses.failed);
  ...
  it("Bug Tackle-966: Migrator, Rules from public and private(with and without credentials)repository", function () {
    migrator.login();
    analyzeAndVerify(analysisWithPublicRules, AnalysisStatuses.completed);
    analyzeAndVerify(analysisWithPrivateRules, AnalysisStatuses.completed);
    analyzeAndVerify(analysisWithPrivateRulesNoCred, AnalysisStatuses.failed);
  ...
  const analyzeAndVerify = (
    analysis: Analysis,
    expectedStatus: AnalysisStatuses
  ) => {
    analysis.analyze();
    /**
