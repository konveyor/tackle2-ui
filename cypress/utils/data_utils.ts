...
export function getAppName(): string {
  ...
  const random_word = getRandomWords(1);
  const random_num = getRandomNumber(1, 200);
...
export function randomWordGenerator(length: number): string {
  ...
  for (let i = 0; i < length; i++) {
    generatedWord += charsToUse.charAt(
  ...
  if (type === CredentialType.sourceControl) {
    ...
    if (useTestingAccount) {
      writeMavenSettingsFile(
        Cypress.env("git_user"),
        Cypress.env("git_password"),
        url
      );
    }
    return {
...
export function getJiraCredentialData(
  ...
  const accountName = "Jira_" + getRandomWord(6);
...
export function getRulesData(
  targetData
): RulesRepositoryFields | RulesManualFields {
