...
import { Application, TaskData, Taskgroup, TaskgroupTask } from '@app/api/models';
...
export const AnalysisWizard: React.FunctionComponent<IAnalysisWizard> = ({
  ...
  const { mutate: deleteTaskgroup } = useDeleteTaskgroupMutation(onDeleteTaskgroupError);
  ...
  const areApplicationsBinaryEnabled = (): boolean =>
    applications.every(
      (application) => application.binary !== '::' && application.binary?.match(/.+:.+:.+/)
    );

  const areApplicationsSourceCodeEnabled = (): boolean =>
    applications.every(
      (application) => application.repository && application.repository.url !== ''
    );
  ...
  const isModeValid = (): boolean => {
    ...
    else if (mode.includes('dependencies')) return areApplicationsSourceCodeDepsEnabled();
