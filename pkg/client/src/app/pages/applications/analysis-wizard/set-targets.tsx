...
export const SetTargets: React.FunctionComponent = () => {
  ...
  const handleOnCardChange = (
    ...
    const selectedTargets = targets.filter((target) => !card.options.has(target));
  ...
  return (
    <>
      <TextContent>
        ...
        <Text>Select one or more target options in focus for the analysis.</Text>
      </TextContent>
      <Stack>
        <StackItem>
          <Gallery hasGutter>
            {options.map((elem, index) => (
              <GalleryItem key={index}>
                <SelectCard
                  item={elem}
                  cardSelected={[...elem.options.keys()].some((key) => targets.includes(key))}
