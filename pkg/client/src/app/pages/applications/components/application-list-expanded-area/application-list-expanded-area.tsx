...
export const ApplicationListExpandedArea: React.FC<IApplicationListExpandedAreaProps> = ({
  ...
  const notYetReviewed = <EmptyTextMessage message={t('terms.notYetReviewed')} />;

  const [appReview, setAppReview] = useState<Review>();
  useEffect(() => {
    const appReview = reviews?.find((review) => review.id === application?.review?.id);
  ...
  return (
    <DescriptionList isHorizontal>
      ...
      <DescriptionListGroup>
        <DescriptionListTerm>{t('terms.businessCriticality')}</DescriptionListTerm>
