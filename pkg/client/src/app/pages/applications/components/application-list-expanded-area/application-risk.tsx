...
export const ApplicationRisk: React.FC<IApplicationRiskProps> = ({ application }) => {
  // Risk
  const fetchRiskData = useCallback(() => {
    return getAssessmentLandscape([application.id!]).then(({ data }) => data[0]);
  }, [application]);

  const { data: applicationRisk, requestFetch: fetchRisk } = useFetch<AssessmentRisk>({
  ...
  return <RiskLabel risk={applicationRisk ? applicationRisk.risk : 'UNKNOWN'} />;
};
