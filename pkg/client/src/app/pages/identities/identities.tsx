...
import { cellWidth, expandable, ICell, IRow, sortable, TableText } from '@patternfly/react-table';
...
import { useDeleteIdentityMutation, useFetchIdentities } from '@app/queries/identities';
...
export const Identities: React.FunctionComponent = () => {
  ...
  const { identities, isFetching, fetchError: fetchErrorIdentities } = useFetchIdentities();
  ...
  currentPageItems?.forEach((item: Identity) => {
    const typeFormattedString = typeOptions.find((type) => type.key === item.kind);
    rows.push({
      [ENTITY_FIELD]: item,
      cells: [
        {
        ...
        {
          title: <TableText wrapModifier="truncate">{typeFormattedString?.value}</TableText>,
        },
        {
